...
% Changing initial condition to see if I can produce a characteristic
% Turing pattern.
...
% Creating a 3x3 grid of "drops".
d=7;
n=0.75;
v0=exp(-((x)^2+(y-n)^2)) + 3*exp(-((x-d+(n*2)^2)^2+(y)^2)) + exp(-((x)^2+(y-d)^2)) + exp(-((x-d)^2+(y-d)^2)) + ...
    2*exp(-((x+d)^2+(y+n)^2)) + exp(-((x)^2+(y+d)^2)) + exp(-((x+(2*n)+d)^2+(y+d)^2)) + ...
    exp(-((x+d-n^2)^2+(y-d+(2*n))^2)) + 2*exp(-((x-d)^2+(y+d)^2));
end
